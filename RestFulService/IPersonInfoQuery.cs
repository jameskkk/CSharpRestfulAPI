...
{
    /// <summary>
    /// 定義兩種方法，1、GetScore方法：通過GET請求傳入name，返回對應的成績；2、GetInfo方法：通過POST請求，傳入Info對象，查找對應的User並返回給Client端
    /// </summary>
    ...
    public interface IPersonInfoQuery
    {
        ...
        /// RequestFormat規定 Client端必須是什麽數據格式請求的（JSon或者XML），不設置默認為XML
        /// ResponseFormat規定Server端返回給 Client端是以是什麽數據格返回的（JSon或者XML）
        /// </summary>
        ...
        /// RequestFormat規定Client端必須是什麽數據格式請求的（JSon或者XML），不設置默認為XML
        /// ResponseFormat規定Server端返回給 Client端是以是什麽數據格返回的（JSon或者XML）
        /// </summary>
